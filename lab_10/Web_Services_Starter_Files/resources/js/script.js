...
//run when the document object model is ready for javascript code to execute  5bc82451636190abd9d7afe6fe9b20b5
$(document).ready(function () {
    var url = 'https://api.weatherstack.com/forecast?access_key=5bc82451636190abd9d7afe6fe9b20b5&query=40,-105.3&forecast_days=5'; //Placeyour weatherstack API Call Here - access_key to be used: 5bc82451636190abd9d7afe6fe9b20b5
    ...
    $.ajax({ url: url, dataType: "jsonp" }).then(function (data) {
        ...
        document.getElementById('image_today').innerText = data.current.weather_icons;
        document.getElementById('heading').innerText = 'Weather Forecast - ' + data.location.name;
        document.getElementById('temp_today').innerText = data.current.temperature + ' F';
        document.getElementById('thermometer_inner').height = data.current.temperature;
        ...
        document.getElementById('summary_today').innerText = data.current.weather_description;

        if (data.current.temperature < 65) {
            document.getElementById('thermometer_inner').style.backgroundColor = "blue";
        }
        else if (data.current.temperature > 85) {
        ...
        //document.getElementById('local_time').innerText = data.current.observation_time;
        /*
...
*/
        ...
        */
        ...
        for (i = 0; i < 5; i++) {
            ...
            divStyle.setAttribute("style", "width: 20%");
            ...
            head.innerText = key.date + ':';
            var p = document.createElement('p');
            p.innerText = 'High: ' + key.maxtemp + '\nLow: ' + key.mintemp + '\nSunrise: ' + key.astro.sunrise + '\nSunset: ' + key.astro.sunset;
            ...
            divStyle.appendChild(divCard);
            document.getElementById('5_day_forecast').appendChild(divStyle);
        }
    })
})